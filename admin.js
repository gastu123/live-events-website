...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function promptFields(fields, title = "Complete details", submitLabel = "Save") {
    return new Promise((resolve) => {
      ...
      fields.forEach(([key, label, initial = "", type]) => {
        ...
        if (!multiline) input.type = type || (/email/i.test(key) ? "email" : /password/i.test(key) ? "password" : "text");
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (action === "assign-details") {
      const amount = (Number(control.dataset.amount) / 100).toFixed(2);
      const giftCard = control.dataset.method === "gift_card";
      const fields = giftCard
        ? [
            ["paymentMethod", "Payment method", control.dataset.method],
            [
              "expiresAt",
        ...
        : [
            [
              "paymentMethod",
              "Payment method: paypal, cash_app, chime or bank_transfer",
              control.dataset.method,
            ],
            ["accountName", "Receiving account name"],
            [
              "paymentIdentifier",
              "PayPal email, $Cashtag, Chime/ACH details or bank identifier",
            ],
            ["amount", "Exact amount", amount],
            ["currency", "Currency", control.dataset.currency],
            ["paymentReference", "Unique customer payment reference"],
      ...
      const values = await promptFields(fields, giftCard ? "Assign payment method" : "Complete details", giftCard ? "Assign Gift Card" : "Save");
      if (values) {
        const amountMinor = Math.round(Number(values.amount) * 100);
        return mutate(
          `/admin/payments/${control.dataset.id}/assign-details`,
          { ...values, amountMinor, currency: control.dataset.currency },
          "payments",
